...
void GenInput_17Bnn(Int_t Eh,Int_t Ah){
  ...
  TFile *file = new TFile(Form("./root/Gen/17BnnInp%02d_%02d.root",Eh,Ah),"recreate");
  ...
  //Set Scat Angle
  TFile *fileparam = new TFile(Form("./root/Gen/17BnnInpParam%02d_%02d.root",Eh,Ah),"recreate");
  ...
  TH1* hscat_in_d = new TH1D("hscat_in_d","Scattering Angle Input [degree]",200,0,10);
  TH1* hscat_in_r = new TH1D("hscat_in_r","Scattering Angle Input [rad]",200,0,0.2);
  ...
  for (int i=0;i<1000000;++i){
    ...
    Int_t A_high = Ah%10;
    if(A_high==0) A_high = 10;

    Erel_in = gRandom->Uniform(E_high-1,E_high);
    herel_in->Fill(Erel_in);
    scat_deg = gRandom->Uniform(A_high-1,A_high);//Degree
