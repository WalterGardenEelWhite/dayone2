...
Double_t Erel_in_max = 10.;//MeV
Double_t Scat_in_max = 0.03;//rad
Double_t cosScat_in_max = cos(Scat_in_max);
Int_t simnum = 10000000;

void GenInput_15Bnn_1(){
  ...
  TFile *file = new TFile("./root/Gen/15BnnInp_1.root","recreate");
  ...
  treeinp->Branch("scat_cos", &scat_cos, "scat_cos/D");
  ...
  TH1* hscat_in_c = new TH1D("hscat_in_d","Scattering Angle Input [cos]",200,cosScat_in_max,1);
  TH1* hscat_in_r = new TH1D("hscat_in_r","Scattering Angle Input [rad]",200,0,Scat_in_max);
  ...
  TH2* h1 = new  TH2D("h1","Erel_in : Scat_in",200,0,Erel_in_max,200,0,Scat_in_max);
  ...
  for (int i=0;i<simnum;++i){
    ...
    herel_in -> GetYaxis() -> SetRangeUser(0,60000);
    //Scattering angle generation
    scat_cos = gRandom->Uniform(cosScat_in_max,1);
    scat_ang = acos(scat_cos);//rad
     
    hscat_in_c->Fill(scat_cos);
    hscat_in_c -> GetYaxis() -> SetRangeUser(0,60000);
    ...
    herel_gen -> GetYaxis() -> SetRangeUser(0,60000);
