...
void AnaNEBTree(){
  ...
  //OUTPUT TTree file-----------------------------------------------------------------------------------------------
  TFile *outfile = new TFile("./root/NEB/AnaNeb16Bnn_V2.root","recreate");
  ...
  //tree->Branch("NEBMultiCut", &NEBMultiCut,"NEBMultiCut/I");
  ...
  for (Int_t ieve=0;ieve<neve;++ieve){
    ...
    if(fNEBV1Multi==0) continue;//||fNEBV2Multi==0)continue;//Remove 1st VETO hit
      for(Int_t i=0;i<144;++i){
   ...
   //QAve & TOF Min gate--------------------------------------------------------------------------
	...
	if( fNEBLayerCut[index[i]]>0 && fNEBLayerCut[index[j]]>0 ){//No gamma ray
	  ...
	  //fNEBdr[NEBMultiCut] = sqrt(pow(fNEBXCut[index[j]] - fNEBXCut[index[i]], 2.)+pow(fNEBYCut[index[j]] - fNEBYCut[index[i]], 2.)+pow(fNEBZCut[index[j]] - fNEBZCut[index[i]], 2.));
          ...
          //if((pow((fNEBdr[NEBMultiCut]-200.0)/(400.0),2) + pow((fNEBdt[NEBMultiCut])/(12.0),2))<1.){//oval
