...
void PlotNEBTree(){
  ...
  tree -> Add("./root/NEB/AnaNeb16Bn_d.root");
  tree -> SetName("tree");

  const Double_t nan = TMath::QuietNaN();
  TFile *outfile = new TFile("./root/Plot/run16Bn_d.root","recreate");
  ...
  TH1D * h0 = new TH1D("h0","Erel 1n",50,0,10);
  ...
  TH1D * h7 = new TH1D("h7","NEB Multipliticy samewall",5,0.5,5.5);
  TH1D * h8 = new TH1D("h8","NEB Multipliticy diffwall",5,0.5,5.5);
  TH1D * h9 = new TH1D("h9","NEB Multipliticy all wall",5,0.5,5.5);
  TH1D * h10 = new TH1D("h10","NEB Multipliticy nogate",5,0.5,5.5);

  TH2D * r1 = new TH2D("r1","Rcross:Erel same wall",50,0,10,50,0,10);
  TH2D * r2 = new TH2D("r2","Rcross:Erel diff wall",50,0,10,50,0,10);
  TH2D * r3 = new TH2D("r3","Rcross:Erel all walls",50,0,10,50,0,10);
  ...
  for (Int_t ieve=0;ieve<neve;++ieve){
    ...
    h10 -> Fill(fNEBINVmulti);
    if(fNEBINVmulti>0){
      Double_t Multi1 = h10->GetEntries();
      //c1->Fill(fNEBdrcut[],fNEBdtcut[]);

    }
    if(fNEBINVmulti>1){
      Double_t Multi2 = h10->GetEntries();
      //c2->Fill(fNEBdrcut[],fNEBdtcut[]);
    }
    Double_t Rcross = Multi2/Multi1;
    //cout << "Rcross = " << Rcross << endl;


    h0->Fill(E_rel_15Bn);
    if(fNEBWallbit<3 && fNEBWallbit>0){
      d1 -> Fill(E_rel_15Bnn_in,Scat_17B_15Bnn_in);//s ameeff
      d4 -> Fill(E_rel_15Bnn_in,Erel_gen);
      h1 -> Fill(Scat_17B_15Bnn_in);
      h2 -> Fill(E_rel_15Bnn_in);
      h7 -> Fill(fNEBINVmulti);
      r1 -> Fill(E_rel_15Bnn,Rcross);
    }
    if(fNEBWallbit==3){
      d2 -> Fill(E_rel_15Bnn_in,Scat_17B_15Bnn_in);//diffeff
      d5 -> Fill(E_rel_15Bnn_in,Erel_gen);
      h3 -> Fill(Scat_17B_15Bnn_in);
      h4 -> Fill(E_rel_15Bnn_in);
      h8 -> Fill(fNEBINVmulti);
      r2 -> Fill(E_rel_15Bnn,Rcross);
    }
    if(fNEBWallbit>0 && fNEBWallbit<4){
      d3 -> Fill(E_rel_15Bnn_in,Scat_17B_15Bnn_in);//sumeff
      h5 -> Fill(Scat_17B_15Bnn_in);
      h6 -> Fill(E_rel_15Bnn_in);
      h9 -> Fill(fNEBINVmulti);
      r3 -> Fill(E_rel_15Bnn,Rcross);
    }
