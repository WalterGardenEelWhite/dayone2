...
void AnaNEBTree_a(){
  ...
  //OUTPUT TTree file-----------------------------------------------------------------------------------------------
  TFile *outfile = new TFile("./root/NEB/AnaNeb16Bn_a.root","recreate");
   ...
   //QAve & TOF Min gate--------------------------------------------------------------------------
	...
	if( fNEBLayerCut[index[i]]>0 && fNEBLayerCut[index[j]]>0 ){//No gamma ray
	  ...
	  //if((pow((fNEBdr[NEBMultiCut]-100.1)/3./63.34,2) + pow((fNEBdt[NEBMultiCut]-0.4847)/3./0.3552,2))<1.){//15B+2n 230110
          //if((pow((fNEBdr[NEBMultiCut]-95.35)/5./64.92,2) + pow((fNEBdt[NEBMultiCut]-0.4881)/5./0.3537,2))<1.){//15B+2n 230719   
	  // if((pow((fNEBdr[NEBMultiCut]-95.35)/4./64.92,2) + pow((fNEBdt[NEBMultiCut]-0.4881)/4./0.3537,2))<1.){//15B+2n 231003
