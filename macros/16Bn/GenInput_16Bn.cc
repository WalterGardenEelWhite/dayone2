...
//Double_t Erel_in_max = 1.;//[MeV]
Double_t Scat_in_max = 30.;//[mrad]
Int_t simnum = 1000000;

void GenInput_16Bn(Int_t Eh){
  ...
  const Double_t Masses[Ndecay] = {m_b15, m_n};

  // Init Phase Space Decay-------------------------------------------------------------------------------------

  TFile *file = new TFile(Form("./root/Gen/16BnInp%02d.root",Eh),"recreate");
  ...
  TFile *fileparam = new TFile(Form("./root/Gen/16BnInpParam%02d.root",Eh),"recreate");
  ...
  for (int i=0;i<simnum;++i){
    ...
    Double_t Ein = m_b16/sqrt(1-randbeta*randbeta) - m_b16;
