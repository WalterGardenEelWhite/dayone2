...
#endif
//void AnaNEBTree(Int_t run){
void AnaNEBTree_b(){

  TChain* oritree = new TChain("tree","ALL tree clone");
  oritree -> Add("./root/Ana/AnaSimn.root");
  ...
  //OUTPUT TTree file-----------------------------------------------------------------------------------------------
  TFile *outfile = new TFile("./root/NEB/AnaNeb16Bn_b.root","recreate");
  ...
  const Double_t m_b19=19.*m_u+58.777;
  const Double_t m_b17=17.*m_u+43.770816;
  const Double_t m_b16=16.*m_u+37.111699;
  const Double_t m_b15=15.*m_u+28.957988;
  ...
  for (Int_t ieve=0;ieve<neve;++ieve){
    ...
    // if(fNEBV1Multi==0||fNEBV2Multi==0)continue;//Remove 1st VETO hit
    if(fNEBV1Multi==0) continue;  
    for(Int_t i=0;i<144;++i){
   ...
   //QAve & TOF Min gate--------------------------------------------------------------------------
    ...
    for(Int_t i=132;i<144;i++){
      if(fNEBTOFCut[i]>0){
        for(Int_t j=60;j<120;j++){//dr(xy)<50cm,1ns<dt(xy)<5ns
	...
	if( fNEBLayerCut[index[i]]>0 && fNEBLayerCut[index[j]]>0 ){//No gamma ray
	  if( (fNEBLayerCut[index[i]]<3  && fNEBLayerCut[index[j]]<3) || (fNEBLayerCut[index[i]]>2 && fNEBLayerCut[index[j]]>2)){//only samewall
	  //if( (fNEBLayerCut[index[i]]<3  && fNEBLayerCut[index[j]]>2) || (fNEBLayerCut[index[i]]>2 && fNEBLayerCut[index[j]]<3)){//only diffwall
	    ...
	    fNEBdr[NEBMultiCut] = sqrt(pow(fNEBXCut[index[j]] - fNEBXCut[index[i]], 2.)+pow(fNEBYCut[index[j]] - fNEBYCut[index[i]], 2.));
	  ...
	  //if((pow((fNEBdr[NEBMultiCut]-94.65)/(5.*78.23),2) + pow((fNEBdt[NEBMultiCut]-0.6534)/(5.*0.3825),2))<1.){//231012 
    ...
    for(Int_t i=0;i<W1NEBCut-1;i++){
      for(Int_t j=i+1;j<W1NEBCut;j++){
        ...
        //-----------------------------------------------------------------------------------------------------------------------------------------------
        if(fNEBLayerCut[w1index[i]]>0&&fNEBLayerCut[w1index[j]]>0){
	    if((fNEBLayerCut[w1index[i]] < 3 && fNEBLayerCut[w1index[j]] < 3) || ( fNEBLayerCut[w1index[i]] > 2 && fNEBLayerCut[w1index[j]] > 2)){//SAME WALL
	      ...
	      //if(fWQ2<(6.-40.)/(1.5-1.)*(fWbeta/fWdbeta+1.)+6. && fWQ2<(40.-6.)/(6.-1.5)*(fWbeta/fWdbeta+6.)+6.){
	      //if(fWQ2<(6.-20.)/(2.-1.)*(fWbeta/fWdbeta+1.)+6. && fWQ2<(20.-6.)/(4.-2.)*(fWbeta/fWdbeta+4.)+6.){//left   
	      //if((fWbeta/fWdbeta)>1. || (fWbeta/fWdbeta)<-1.5){
	      //  SWallcutbit[w1index[j]] = 1;
	      //  fW1betacut[W1NEBCut] = nan;
	      //  fW1dbetacut[W1NEBCut] = nan; 
	      //}
	      //if(fWQ2<(50.-6.)/(1.5-1.)*(fWbeta/fWdbeta-1.)+6. && fWQ2<(6.-50.)/(6.-1.5)*(fWbeta/fWdbeta-6.)+6.){
	      //if(fWQ2<(40.-6.)/(1.5-1.)*(fWbeta/fWdbeta-1.)+6. && fWQ2<(6.-40.)/(4.-1.5)*(fWbeta/fWdbeta-4.)+6.){//right 
	      //SWallcutbit[w1index[j]] = 1;
	      //fW1betacut[W1NEBCut] = nan;
	      //fW1dbetacut[W1NEBCut] = nan; 
	      //}
	      //if(fabs(fabs(fWdbeta)-1)<3.*0.2393 && fWQ2<15){
	      //  SWallcutbit[w1index[j]] = 1;
	      //  fW1betacut[W1NEBCut] = nan;
	      //  fW1dbetacut[W1NEBCut] = nan; 
	      //}
	    ...
	    //-----------------------------------------------------------------------------------------------------------------------------------------------
	      ...
	      // if(((1./fWdbeta)>-1.612&&(1./fWdbeta)<-0.227)&&fWQ2<20.){
	      //   SWallcutbit[w1index[j]] = 1;
	      //   fW2betacut[W1NEBCut] = nan;
	      //   fW2dbetacut[W1NEBCut] = nan;
	      // }
	      // if(((1./fWdbeta)>0.378&&(1./fWdbeta)<1.604)&&fWQ2<20.){
	      //   SWallcutbit[w1index[j]] = 1;
	      //   fW2betacut[W1NEBCut] = nan;
	      //   fW2dbetacut[W1NEBCut] = nan;
	      // }
	      
	      ////if(fWQ1<182.5*fWbeta/fWdbeta -131.12){
	      //if(fWbeta/fWdbeta<-1.5){//left              
	      //  SWallcutbit[w1index[j]] = 1;
	      //  fW2betacut[W1NEBCut] = nan;
	      //  fW2dbetacut[W1NEBCut] = nan; 
	      //}
	      ////if(fabs(fabs(fWdbeta)-1.)<3.*0.09962 && fWQ2<15){
	      //if(fWbeta/fWdbeta>1.){//right  
	      //  SWallcutbit[w1index[j]] = 1;
	      //  fW2betacut[W1NEBCut] = nan;
	      //  fW2dbetacut[W1NEBCut] = nan; 
	      //}
	      //if(fWbeta/fWdbeta<-1.5){
	      //  SWallcutbit[w1index[j]] = 1;
	      //  fW2betacut[W1NEBCut] = nan;
	      //  fW2dbetacut[W1NEBCut] = nan; 
	      //}
    ...
    if(fNEBLayerCut[invindex[0]]>0 && fNEBLayerCut[invindex[1]]>0 && fNEBINVmulti > 1){
      ...
      TLorentzVector vecf(Fgen_px,Fgen_py,Fgen_pz,Fgen_E);
        ...
        Mass_inv_15Bnn = (vec1+vec2+vecf).M(); 
        E_rel_15Bnn = Mass_inv_15Bnn  - m_b15 - m_n - m_n;
	E_rel_15Bnn_in = Erel_in;//uniformly generated        

	// if(fNEBWallbit==3)cout << ieve << " " << fNEBWallbit << " " << E_rel_15Bnn << endl;
	...
	Scat_17B_15Bnn_in = scat_ang;//uniformly gnenrated
