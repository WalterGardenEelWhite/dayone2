...
//void AnaOutput_15Bnn(Int_t Eh){
void AnaOutput_n(){

  TFile *file = new TFile("./root/Sim/Simn0000.root","readonly");
  ...
  paramtree -> Add("./root/Gen/nInpParam.root");
  ...
  TFile* outfile = new TFile("./root/Ana/AnaSimn.root","recreate");
  ...
  TH2* hbeamposxy_gated = new TH2D("hbeamposxy_gated","Beam Pos XY (mm) gated",200,-50,50,200,-50,50);
  ...
  for (Int_t ieve=0;ieve<neve;++ieve){
    ...
    for(Int_t ipla=0;ipla<npla;++ipla){
      ...
      fNEBZ[ID-1] = pla -> GetPos(2) - tgtz;//[mm]
      fNEBLgt[ID-1] = sqrt(pow(fNEBX[ID-1]-tgtx, 2.) + pow(fNEBY[ID-1]-tgty, 2.) + pow(fNEBZ[ID-1], 2.));//[mm]
